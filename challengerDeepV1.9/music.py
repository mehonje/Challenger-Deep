...
class Music(Sprite):
    def __init__(self, gameEngine, costume = None):
        ...
        self.songs = ["songs/hammockTurnAwayAndReturn.mp3", "songs/scottBuckleyAurora.mp3", "songs/erikWernquistCassini'sGrandFinale.mp3", "songs/cristianSandquistWanderers.mp3", "songs/scottBuckleyTheSpacesBetween.mp3", "songs/scottBuckleyTheRestoration.mp3", "songs/scottBuckleyTheDistantSun.mp3", "songs/scottBuckleyChasingDaylight.mp3", "songs/alekseyChistilinOcean.mp3", "songs/stanislavBarsantovInterstellarTravelling.mp3", "songs/M83AnotherWaveFromYou.mp3"]
        self.songNames = ["Turn Away And Return - Hammock", "Aurora - Scott Buckley", "Cassini's Grand Finale - Erik Wernquist", "Wanderers - Cristian Sandquist", "The Spaces Between - Scott Buckley", "The Restoration - Scott Buckley", "The Distant Sun - Scott Buckley", "Chasing Daylight - Scott Buckley", "Ocean - Aleksey Chistilin", "Interstellar Travelling - Stanislav Barsantov", "Another Wave From You - M83"]
    ...
    def tick(self):
        if self.phase == 0 and self.gameEngine.menu == 0:
            self.phase = 1
            self.playSong(0)

        if self.phase == 1 and self.gameEngine.playerDepth >= 200:
        ...
        if self.phase == 2 and self.gameEngine.playerDepth >= 1000:
        ...
        if self.phase == 3 and not pygame.mixer.music.get_busy():
            pygame.mixer.music.set_volume(pygame.mixer.music.get_volume() -(self.gameEngine.timers.deltaTime / 10))
        ...
        if self.phase == 3 and self.gameEngine.playerDepth >= (10914 - (abs(self.gameEngine.player.yVelocty * 87.67))) - 1000:
            self.phase = 4

        if self.phase == 4 and self.gameEngine.playerDepth >= 10914 - ((abs(self.gameEngine.player.yVelocity * 0.1) * 71.13)):
            pygame.mixer.music.set_volume(pygame.mixer.music.get_volume() - (self.gameEngine.timers.deltaTime / 10))
        ...
        if self.phase == 5 and self.gameEngine.ballastEnabled == 0:
            self.phase = 6
            self.playSong(8)

        if self.phase == 6 and self.gameEngine.playerDepth <= (abs(self.gameEngine.player.yVelocity * 0.1) * 113.8):
            pygame.mixer.music.set_volume(pygame.mixer.music.get_volume() - (self.gameEngine.timers.deltaTime / 10))
